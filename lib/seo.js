...
export function organizationSchema(locale, settings = {}) {
  ...
  const schema = {
    ...
    sameAs: [
      "https://www.instagram.com/alusraclinics",
      "https://www.snapchat.com/add/alusraclinics",
      "https://www.facebook.com/alusraclinics",
      "https://x.com/alusraclinics",
    ],
  };

  if (settings?.aggregateRating) {
  ...
  if (settings?.reviews?.length) {
    schema.review = settings.reviews.slice(0, 5).map((r) => ({
      "@type": "Review",
