...
export const getServices = ttlCached("services", (args) => {
  const { category, includeSubTypes = false } = args || {};
  return safeQuery((supabase) => {
    let query = supabase.from("services").select("*").eq("active", true).order("sort_order");
    if (!includeSubTypes) query = query.is("parent_service_id", null);
    if (category) query = query.eq("category", category);
    return query;
  }, fallbackServices.filter((s) => (includeSubTypes || !s.parent_service_id) && (!category || s.category === category)));
});

export const getServiceSubTypes = ttlCached("serviceSubTypes", (args) => {
  const { serviceId, serviceSlug, activeOnly = true } = args || {};
  if (!serviceId && !serviceSlug) return [];
  const fallbackMatches = fallbackServices.filter(
    (s) => s.parent_service_id !== undefined && (!activeOnly || s.active)
  );
  const fallbackParent = fallbackMatches.find(
    (s) => s.slug === serviceSlug && s.parent_service_id === undefined
  );
  const fallback = fallbackParent
    ? fallbackMatches.filter((s) => s.parent_service_id === fallbackParent.id)
    : [];
  return safeQuery((supabase) => {
    let query = supabase
      .from("services")
...
export const getServiceSubTypeBySlug = ttlCached("serviceSubTypeBySlug", async (args) => {
  const { parentSlug, subTypeSlug } = args || {};
  const parent = await getServiceBySlug(parentSlug);
  if (!parent?.id || !subTypeSlug) return null;
  const fallbackMatch =
    fallbackServices.find(
      (s) =>
        s.slug === subTypeSlug &&
        fallbackServices.some((p) => p.id === s.parent_service_id && p.slug === parentSlug)
    ) || null;
  const data = await safeQuery(
    (supabase) =>
      supabase
        ...
        .select("*")
        .eq("slug", subTypeSlug)
        .eq("parent_service_id", parent.id)
        .eq("active", true)
        .maybeSingle(),
    fallbackMatch
  );
